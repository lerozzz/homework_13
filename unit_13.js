...
function f5(arr, elementName) {
    const element = document.querySelector(elementName);
    for (let key in arr) {
        element.innerHTML += `${key}: ${arr[key]} <br>`;
    }

    // цикл
    // формат вывода `${key} : ${arr[key]} <br>`;
    //
    // тут вывод в блок block//elementName
}
...
function f13() {
    ...
    for (let key in a13) {
        console.log(a13[key]);
        // console.log(typeof typeof a13[key]);
        // typeof a13[key] === "number"
...
function f16() {
    let out = '';
    for (let keyMain in a16) {
        out += a16[keyMain].name + ' ';
    }
...
function f17() {
    let out = '';
    for (let keyMain in a17) {
        if (a17[keyMain].age > 30) {
            out += a17[keyMain].name + ' ';
        }
    }
